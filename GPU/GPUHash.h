...
__device__ __noinline__ void _GetHash160Comp(uint64_t *x, uint8_t isOdd, uint8_t *hash) {
  ...
  /*
  hash[0] = (publicKeyBytes[0] >> 16);
  hash[1] = (publicKeyBytes[0] >> 8);
  hash[2] = publicKeyBytes[0];
  hash[3] = (publicKeyBytes[1] >> 24);
  
  hash[4] = (publicKeyBytes[1] >> 16);
  hash[5] = (publicKeyBytes[1] >> 8);
  hash[6] = publicKeyBytes[1];
  hash[7] = (publicKeyBytes[2] >> 24);
  
  hash[8] = (publicKeyBytes[2] >> 16);
  hash[9] = (publicKeyBytes[2] >> 8);
  hash[10] = publicKeyBytes[2];
  hash[11] = (publicKeyBytes[3] >> 24);
  
  hash[12] = (publicKeyBytes[3] >> 16);
  hash[13] = (publicKeyBytes[3] >> 8);
  hash[14] = publicKeyBytes[3];
  hash[15] = (publicKeyBytes[4] >> 24);
  
  hash[16] = (publicKeyBytes[4] >> 16);
  hash[17] = (publicKeyBytes[4] >> 8);
  hash[18] = publicKeyBytes[4];
  hash[19] = (publicKeyBytes[5] >> 24);
  */
  
  // abracadabra ;)
  int pi = 0;
  for (int i = 0; i < 20; i+=4) {
	hash[i] = (publicKeyBytes[pi] >> 16);
	hash[i+1] = (publicKeyBytes[pi] >> 8);
	hash[i+2] = publicKeyBytes[pi];
	pi += 1;
	hash[i+3] = (publicKeyBytes[pi] >> 24);
  }
  
  //printf("4 bytes point.x: %02x%02x%02x%02x \n", (int)hash[0],(int)hash[1],(int)hash[2],(int)hash[3]);
