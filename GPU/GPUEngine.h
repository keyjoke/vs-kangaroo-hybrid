...
public:
  ...
  GPUEngine(int nbThreadGroup, int gpuId, uint32_t maxFound, bool rekey, uint32_t pow2w, uint32_t hop_modulo, int power); 
  ~GPUEngine();
  ...
  bool SetKeys(Point *p, Point *wp, Int *keys, Int *wkeys);
...
private:
  ...
  uint64_t *w_inputKey;// Wild point
  uint64_t *w_inputKeyPinned;
  
  uint64_t *s_inputKey;// Tame and Wild Start keys
  uint64_t *s_inputKeyPinned;
