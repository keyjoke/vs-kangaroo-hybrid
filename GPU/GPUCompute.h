...
__device__ void ComputeKeys(uint64_t *startx, uint64_t *starty, uint64_t *wstartx, uint64_t *wstarty, uint64_t *TStartKey, uint64_t *WStartKey, 
	...
	uint32_t spin = 64;
	uint32_t j;
	for (j = 0; j < spin; j++) { //for (j = 0; j < hop_modulo; j++) { 
	
		if (1) {// check, is it distinguished point ?
			...
			// Get wdx and dx
			ModSub256(_2dx[0], Spx[pw2], wpx);// wild
			ModSub256(_2dx[1], Spx[pw1], px);// tame
			// Compute 2 ModInv
			_ModInvNoGrouped2(_2dx);
			Load256(wdx[pw2], _2dx[0]);// wild
			Load256(dx[pw1], _2dx[1]);// tame
