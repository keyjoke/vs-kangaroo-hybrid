...
void VanitySearch::FindKeyGPU(TH_PARAM *ph) {
  ...
  //printf("\nGPUJmaxofSp: %llu \n", GPUJmaxofSp);
  int hopmod = (int)GPUJmaxofSp;
  Int hopstep(&dS[hopmod]);  
  
  // Global init
  int thId = ph->threadId;
  GPUEngine g(ph->gridSize, ph->gpuId, maxFound, (rekey!=0), pow2W, GPUJmaxofSp, KangPower);
  int nbThread = g.GetNbThread();
  Point *p = new Point[nbThread];// Tame point
  Point *wp = new Point[nbThread];// Wild point
  Int *keys = new Int[nbThread];// Tame start keys
  Int *wkeys = new Int[nbThread];// Wild start keys
  vector<ITEM> found;
  ...
  // Get Tame points
  getGPUStartingKeys(thId, g.GetGroupSize(), nbThread, keys, p, &n_count);
  
  // Get Wild points and keys
  getGPUStartingWKeys(thId, g.GetGroupSize(), nbThread, targetPubKey, wkeys, wp);
  
  ok = g.SetKeys(p, wp, keys, wkeys);//ok = g.SetKeys(p);
  ...
  #ifdef WIN64
  #else
  string comm_chmod = "chmod 755 ./solver.py";
  ...
  while (ok && !flag_endOfSearch) {
	...
	for(int i=0;i<(int)found.size() && !flag_endOfSearch;++i) {
		...
		ITEM it = found[i];
		//checkAddr(*(prefix_t *)(it.hash), it.hash, keys[it.thId], it.incr, it.mode);
		//checkAddr(*(prefix_t *)(it.hash), it.hash, keys[it.thId], it.incr, it.mode, it.thId, keys);		
		
		Int Tpx;
		Tpx.uSet((uint64_t *)(it.tpx));
		Int Tkey;
		Tkey.uSet((uint64_t *)(it.tkey));
		...
		if (it.type == 1 || it.type == 2) {
			...
			if (!f2save) {
				printf("\n[error] Not saved file type: %d \n", (int)it.type);
			}
		}
	}
	
	//printf("\n g_count: %d ", kadd_count);
	...
	if (ok) {
	  ...
	  ++ kadd_count;
	  //countj[thId] += 1ULL * STEP_SIZE * nbThread; // Point only
	  countj[thId] += 64ULL * nbThread; // Point only
    }
...
void VanitySearch::SolverGPU(TH_PARAM *ph) {
	...
	while (!flag_endOfSearch) {
		...
		// Python Comparator wrong ?
		#ifdef WIN64
		string comm_cmp_win = "solver.py";
...
uint64_t VanitySearch::getCountJ() {
  ...
  uint64_t count = 0;
  for(int i = 0 ; i < nbCPUThread ; ++i)//for(int i = 0 ; i < nbThread ; ++i)
    count += countj[i];
  return count;
...
void VanitySearch::Search(bool useGpu, std::vector<int> gpuId, std::vector<int> gridSize) {
  ...
  //nbThread = (useGpu ? (int)gpuId.size() : nbThread);
...
void VanitySearch::ReWriteFiles(){
	...
	FILE *f1 = fopen("tame.txt", "w");
	fclose(f1);
	FILE *f2 = fopen("wild.txt", "w");
	fclose(f2);
